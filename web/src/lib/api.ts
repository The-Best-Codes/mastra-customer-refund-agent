...
export function submitCase(payload: MockEmailPayload): Promise<{ caseId: string; workflowRunId?: string; status: string }> {
  return request('/support/inbound', { method: 'POST', body: JSON.stringify(payload) });
}

export function approveCase(caseId: string, approverId: string, note?: string): Promise<SupportCase> {
  return request(`/support/cases/${caseId}/approve`, { method: 'POST', body: JSON.stringify({ approverId, note }) });
}

export function rejectCase(caseId: string, approverId: string, note?: string): Promise<SupportCase> {
  return request(`/support/cases/${caseId}/reject`, { method: 'POST', body: JSON.stringify({ approverId, note }) });
}
